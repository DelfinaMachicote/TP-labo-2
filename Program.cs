...
namespace TP_labo_2
{
    ...
    class Program
    {
        ...
        public static void Asignar(Reina reina, Rey rey, Torre t1, Torre t2, Alfil a1, Alfil a2, Caballo c1, Caballo c2, bool[,] tablero)
        {//pasamos todas las piezas por separado porque salta error cada vez que casteamos la clase padre


            Random rnd = new Random();

            bool[,] aux = new bool[8, 8];
        ...
        public static void Imprimir(bool[,] tablero)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    ...
                    if (tablero[j, i] == true) { Console.Write(1 + "   "); }

                    if (tablero[j, i] == false) { Console.Write(0 + "   "); }
